...
async def run(body: RunRequest) -> RunResult:
    ...
    # Pass the code to the child container via an env var instead of bind-
    # mounting a tempfile. Bind-mounting requires an absolute host path, which
    # depends on how the sandbox service is deployed (${PWD} in compose can
    # resolve to a relative/non-existent path on some hosts and Docker rejects
    # it: 'invalid characters for a local volume name'). Env vars sidestep
    # that entirely. The code is still written into the conversation's
    # workspace volume so it survives across turns.
    code = body.code.replace("\r\n", "\n")
    code_b64 = base64.b64encode(code.encode("utf-8")).decode("ascii")
    env = dict(body.env)
    env["HESTIA_CODE"] = code_b64

    command = (
        "python3 -c \"import base64,os;"
        "open('/workspace/main.py','w').write("
        "base64.b64decode(os.environ['HESTIA_CODE']).decode())\""
